...
import { collectValues, findFieldForName, copyObjectDeep } from '../../utils/utils.js';
...
export default function AdvertAdd(props){
    ...
    const [fields, setFields] = useState(copyObjectDeep(fieldsPreset));
    ...
    function getData(){
        ...
        let advert = firestore.collection("adverts").doc(locationParams.id);
        advert.get()
        .then((doc) => {
            let data = doc.data()
    ...
    function onClear(){
        setFields(copyObjectDeep(fieldsPreset))
    }
                    ...
                    {loading ? (
                        ...
                        {props.edit && (
                            <div className="form_item form_item-button">
                                <button
                                    className="form_button button button-white"
                                    onClick={() => {
                                        if(!window.confirm('Вы уверены что хотите удалить это объявление?')) return;
                                        setLoading(true)
                                        firestore.collection('adverts').doc(location.id).delete()
                                            .then((res) => {
                                                Alert.success('Объявление удалено')
                                                return <Redirect to={'/'} />
                                            })
                                            .catch((error) => { 
                                                Alert.error(error)    
